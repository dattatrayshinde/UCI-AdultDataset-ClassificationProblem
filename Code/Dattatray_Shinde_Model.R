...
set.seed(seed)
# cross-validation
model_xgb_cv <- xgb.cv(data=as.matrix(train_new), label=as.matrix(target), objective="binary:logistic", nfold=10, nrounds=1200, eta=0.02, max_depth=5, subsample=0.6, colsample_bytree=0.85, min_child_weight=1, eval_metric="auc")
print(model_xgb_cv)

# model building
model_xgb <- xgboost(data=as.matrix(train_new), label=as.matrix(target), objective="binary:logistic", nrounds=1200, eta=0.02, max_depth=5, subsample=0.6, colsample_bytree=0.85, min_child_weight=1, eval_metric="auc")
# model scoring
pred <- predict(model_xgb, as.matrix(test_new))
# submission
